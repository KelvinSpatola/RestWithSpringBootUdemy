...
public class PersonController {
	...
	@ApiOperation(value = "Find all disabled people" ) 
	@GetMapping(value = "/disabled", produces = { "application/json", "application/xml", "application/x-yaml" })
	public List<PersonVO> findAllDisabled() {
		List<PersonVO> persons = service.findAllDisabled();
		persons.stream().forEach(p -> p.add(linkTo(methodOn(PersonController.class).findById(p.getKey())).withSelfRel()));
		return persons;
	}
